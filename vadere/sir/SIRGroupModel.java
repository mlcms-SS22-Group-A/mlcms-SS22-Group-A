...
public class SIRGroupModel extends AbstractGroupModel<SIRGroup> {
	...
	private final double averagePedestrianMovement = 1.42;
	private double simTimeStepLength;
	private boolean simTimeStepLengthSet;
	...
	public void initialize(List<Attributes> attributesList, Domain domain,
		...
		this.simTimeStepLength = 0.4;
		this.simTimeStepLengthSet = false;
	}
	...
	public void update(final double simTimeInSec) {
		// extract the simTimeStepLength of the simulation and set the infectionMaxDistance according to that
		if (!this.simTimeStepLengthSet) {
			this.simTimeStepLength = simTimeInSec;
			attributesSIRG.setInfectionMaxDistance(Math.max(this.averagePedestrianMovement * this.simTimeStepLength, attributesSIRG.getInfectionMaxDistance()));
			this.simTimeStepLengthSet = true;
		}

		// check the positions of all pedestrians and switch groups to INFECTED (or REMOVED).
		DynamicElementContainer<Pedestrian> c = topography.getPedestrianDynamicElements();

		// find the linkedcellsgrid holding the pedestrians
		LinkedCellsGrid<Pedestrian> lcg = topography.getSpatialMap(Pedestrian.class);

		if (c.getElements().size() > 0) {
			for(Pedestrian p : c.getElements()) {
				// No update required for recovered pedestrians
				if (getGroup(p).getID() == SIRType.ID_RECOVERED.ordinal()) {
					continue;
				}
				// if a pedestrian is infected and the random number is smaller than the recovery rate, change state of pedestrian
				// to recovered
				...
				// loop over neighbors and set infected if we are close (we are now using the getObjects() method in LinkedCellsGrid class,
				// which returns the pedestrian objects that are in a circle centered on p.getPostion() and has a radius of
				// attributesSIRG.getInfectionMaxDistance()
				for (Pedestrian p_neighbor : lcg.getObjects(p.getPosition(), attributesSIRG.getInfectionMaxDistance())) {
